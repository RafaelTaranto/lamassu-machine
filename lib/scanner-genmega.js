...
const getInfo = barcodeScannerPath => {
  let { return_int, data } = bcs.getInfo(barcodeScannerPath)
  if (return_int !== 0) {
    console.log('scanner: error getting scanner info', return_int)
    data = ""
  }

  data = data.toLowerCase()
  const isHoneywell = data.includes("honeywell")
  const isNewland = data.includes("newland")
...
const getDefaultExtensions = barcodeScannerPath => {
  const { isHoneywell, isNewland } = getInfo(barcodeScannerPath)
  return isHoneywell ? [] : isNewland ? ["EXPLVL5"] : []
}
...
function config (_configuration) {
  ...
  extensions = _configuration?.extensions ?? getDefaultExtensions(barcodeScannerPath)
}
...
const scanPDF417 = ({ resultCallback }) => {
  gmrunning = true
  bcs.scan(barcodeScannerPath, 1, extensions)
