...
const capture = async ({
  ...
  if (liveviewEnabledCallback)
    liveview.start(liveviewEnabledCallback)
...
const scanPDF417 = ({ resultCallback, stillsCallback }) => {
  ...
  const processCallback = async ({ frame, width, height }) => {
    ...
    const result = await pdf417Scanner.scanPDF417({ frame: bwFrame, width, height })
...
const scanQR = ({ resultCallback }) => {
  ...
  const processCallback = async ({ frame, width, height }) => {
    ...
    return qrScanner.scanQRcode({ frame: bwFrame, encodedFrame: bwEncoded, width, height })
  }

  capture({ device, mode, resultCallback, processCallback })
}

const scanMainQR = ({ resultCallback, cryptoCode, stillsCallback, liveviewEnabledCallback }) => {
  const mode = 'qr'
  const device = getCameraDevice(mode)
  ...
  const processCallback = async ({ frame, width, height }) => {
    ...
    const code = await qrScanner.scanQRcode({ frame: bwFrame, encodedFrame: bwEncoded, width, height })
    if (!code) return null

    return coinUtils.parseUrl(cryptoCode, NETWORK, code)
  }

  capture({ device, mode, resultCallback, stillsCallback, liveviewEnabledCallback, processCallback })
}
