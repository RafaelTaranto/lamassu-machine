...
Id003.prototype.run = function run (cb) {
  ...
  this.rs232 = Rs232.factory(rs232Config)
  ...
  this.id003Fsm.on('failure', function (args) {
    self.emit('error', args)
  })

  this.rs232.open(function (err) {
    if (err) return self._startupCallback(err)
    self._startPolling()
...
Id003.prototype._denominations = function _denominations () {
  return this.rs232?.denominations()
}
