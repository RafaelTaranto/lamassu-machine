...
const TABLE_FIELDS = {
  staticConfig: [
    'version', 'enablePaperWalletOnly', 'hasLightning', 'serverVersion', 'timezone',
  ...
  coins: [
    ...
    'cashInCommission', 'cashOutCommission', 'cashOutFee', 'cryptoNetwork', 'cryptoUnits',
    'batchable', 'isCashInOnly'
  ],
  operatorInfo: ['name', 'phone', 'email', 'website', 'companyNumber'],
  receiptOptions: ['field', 'enabled'],
  screenOptions: ['ratesScreenActive'],
...
const loadConfig = () => {
  return db.transaction(async (trx) => {
    const loaders = {
      ...
      screenOptions: trx('screenOptions').first().queryContext({ booleanColumns: ['ratesScreenActive'] })
        .then(options => ({ rates: { active: options?.ratesScreenActive } }))
    }
...
const saveConfig = async ({
  ...
  return db.transaction(async (trx) => {
    ...
    const saveScreenOptions = replaceAllRows('screenOptions', {
      ratesScreenActive: screenOptions?.rates?.active ?? false
    })

    return Promise.all([saveCoins, saveLocales, saveOperatorInfo, saveReceiptOptions, saveSpeedtestFiles, saveTermsInternal, saveTriggersAutomation, saveURLsToPing, saveStaticConfig, saveTriggers, saveScreenOptions])
  })
}
