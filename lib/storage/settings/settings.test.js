...
describe('coins', () => {
  test('should update commission rates', async ({ save }) => {
    ...
    expect(coins[0].cashOutFee).toBe('0.05')
  })
})
...
describe('receipt options', () => {
  test('should toggle multiple receipt options', async ({ save }) => {
    ...
    expect(receiptOptions.automaticPrint).toBe(true)
  })
  ...
  test('should handle automatic print setting in receipt options', async ({ save }) => {
    const newReceiptOptions = {
      ...save.receiptOptions,
      automaticPrint: true
    }
...
describe('screen options', () => {
  test('should save and load rates screen active state', async ({ save }) => {
    const screenOptions = {
      rates: { active: true }
    }

    await saveConfig({ ...save, screenOptions })
    const { screenOptions: loadedOptions } = await loadConfig()
    expect(loadedOptions).toEqual(screenOptions)
  })

  test('should update rates screen active state', async ({ save }) => {
    await saveConfig(save)
    const updatedScreenOptions = {
      rates: { active: false }
    }

    await saveConfig({ ...save, screenOptions: updatedScreenOptions })
    const { screenOptions } = await loadConfig()
    expect(screenOptions.rates.active).toBe(false)
  })

  test('should handle empty screen options', async ({ save }) => {
    await saveConfig({ ...save, screenOptions: {} })
    const { screenOptions } = await loadConfig()
    expect(screenOptions.rates.active).toBe(false)
  })
