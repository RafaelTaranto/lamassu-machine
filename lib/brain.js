...
const Brain = function (config) {
  ...
  this.shouldUseLiveView = ['coincloud'].includes(config.cryptomatMaker ?? 'lamassu')
...
Brain.prototype._networkDown = function _networkDown () {
  ...
  // Only do so if we're not connecting for the first time
  if (this.hasConnected && (!tx || !tx.direction))
    return this._setNetworkDownTimeout()

  const doForceDown = !this.hasConnected || (tx.direction === 'cashIn' && _.isEmpty(tx.bills)) ||
...
Brain.prototype.postTx = function postTx (tx) {
  ...
  const poll = () => post().then(serverTx => {
    // server protocol is to return empty if you need to retry
    if (serverTx) return serverTx
    return delay(ACTIVE_POLL_INTERVAL).then(poll)
  })

  const polling = poll()

  // Saves to the local db, don't fail if it doesn't work
  // the catch here won't prevent errors from bubbling up since we're racing 'polling' afterward
  polling
    .then(serverTx => this.saveTx(serverTx))
    .catch(() => {})

  // Return to caller: tx or timeout, whichever comes first
  return Promise.race([
    polling,
    delay(NETWORK_TIMEOUT_INTERVAL).then(() => { throw new Error('timeout') })
  ])
}
