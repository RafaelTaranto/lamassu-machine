...
var watchdogInfoLoader = require('../storage/watchdog-info')
...
const execFilePromise = (program, args, opts) => new Promise((resolve, reject) =>
  execFile(
    program, args, opts,
    (err, stdout, stderr) => {
      const std = { stdout, stderr }
      return err ? reject(Object.assign(err, std)) : resolve(std)
    }
  )
)
...
const x11envvars = {
  DISPLAY: ":0",
  XAUTHORITY: "/home/lamassu/.Xauthority",
}

const xrandr = (...args) => execFilePromise("xrandr", [...args], { env: x11envvars })
const getXrandrDetails = (isCC) =>
  isCC ?
    xrandr("--query", "--verbose")
      .then(({ stdout }) =>
        stdout.split('\n')
              .filter(line => line.match(/^[a-zA-Z0-9-]+\s+connected\s+/))
              .map(line => line.replaceAll(/\s+\([^)]+\)/gi, ''))[0]
          ?? "none"
      )
      .catch(err => {
        console.log("Failed to get xrandr details:", err)
        return "unknown"
      }) :
    Promise.resolve("disabled")

const systemctl = (...args) => execFilePromise("systemctl", [...args], { env: x11envvars })
const getLightDMStatus = (isCC) =>
  isCC ?
    systemctl("list-units", "--plain", "lightdm.service")
      .then(({ stdout }) =>
        stdout.split('\n')
              .map(line => line.trim())
              .filter(line => line.startsWith("lightdm.service"))[0]
          ?? "none"
      )
      .catch(err => {
        console.log("Failed to get lightdm status:", err)
        return "unknown"
      }) :
    Promise.resolve("disabled")

Updater.prototype.updateHeaders = function updateHeaders (headers) {
  const dataPath = path.resolve(__dirname, '..', '..', this.config.dataPath)

  const watchdogInfo = watchdogInfoLoader.load(dataPath)
  const machineInfo = machineInfoLoader.load(dataPath)

  const isCC = watchdogInfo.platform === 'coincloud'

  return Promise.all([getXrandrDetails(isCC), getLightDMStatus(isCC)])
    .then(([xrandrDetails, lightdmStatus]) => {
      ...
      headers = _.assign(headers, {
        xrandr: xrandrDetails,
        lightdm: lightdmStatus,
      })
  ...
  return this.updateHeaders(this.httpsOptions.headers)
    .then(headers => {
      ...
      return https.get(this.httpsOptions, res => {
        ...
        const restrictionLevel = parseInt(res.headers['x-restriction-level'] || '0', 10)
        this._saveRestrictionLevel(restrictionLevel)
...
Updater.prototype._saveRestrictionLevel = function _saveRestrictionLevel (level) {
  const dataPath = path.resolve(__dirname, '..', '..', this.config.dataPath)
  
  restrictionInfoLoader.save(dataPath, level)
    .then(() => console.log('Restriction level saved:', level))
    .catch(err => console.error('Failed to save restriction level:', err))
}
